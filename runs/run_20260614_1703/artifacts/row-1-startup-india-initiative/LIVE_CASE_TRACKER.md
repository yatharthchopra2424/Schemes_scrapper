# Live Case Tracker

## Active Cases

| Case ID | Client | Scheme | Consultant | Stage | Submitted | Next Action | Deadline | Est. Value (₹) | Status |
|---|---|---|---|---|---|---|---|---|---|
| CST-001 | [PLACEHOLDER] | Startup India Initiative | [PLACEHOLDER] | Submitted | 01/04 | Follow-up call | Rolling basis — no fixed deadline | Up to INR 20 Lakhs from SISFS for early-stage validation | 🟡 In progress |

---

## Case Detail Sheet

### Case ID: CST-001
**Client:** [PLACEHOLDER]  
**Scheme:** Startup India Initiative (DPIIT Recognition)  
**Target benefit:** ₹[TO BE FILLED BY CONSULTANT]  
**Consultant lead:** [PLACEHOLDER]  
**Support analyst:** [PLACEHOLDER]

#### Timeline Log
| Date | Action | By | Notes |
|---|---|---|---|
| DD/MM | Discovery call | [Name] | Client confirmed eligible for Startup India Initiative |
| DD/MM | Proposal sent | [Name] | Awaiting sign-off |
| DD/MM | Retainer received | [Name] | ₹[TO BE FILLED] |
| DD/MM | Docs collection started | [Name] | [N]/13 docs received |
| DD/MM | Registered on https://www.startupindia.gov.in/ | [Name] | Portal login created |
| DD/MM | Application submitted | [Name] | Ref# XXXXX |

#### Document Tracker
| Document | Received | Quality Check | Notes |
|---|---|---|---|
| Certificate of Incorporation / Registration | ❌ | — |  |
| PAN of the entity | ❌ | — |  |
| Pitch deck or business description | ❌ | — |  |
| Authorization letter from authorized signatory | ❌ | — |  |
| Details of funding received (if any) | ❌ | — |  |
| Memorandum of Association for Pvt. Ltd. / LLP Deed | ❌ | — |  |
| Board Resolution (if any) | ❌ | — |  |
| Annual Accounts of the startup for the last three financial years | ❌ | — |  |
| Income Tax returns for the last three financial years | ❌ | — |  |
| Incorporation number (CIN) | ❌ | — |  |
| LLPIN (if applicable) | ❌ | — |  |
| Registered Mobile Number | ❌ | — |  |
| Email ID of Authorized Representative | ❌ | — |  |

#### Correspondence Log
| Date | Channel | From | To | Summary |
|---|---|---|---|---|
| DD/MM | Email | Us | Department for Promotion of Industry and Internal Trade (DPIIT), Ministry of Commerce and Industry | Requested status update |

#### Risks & Blockers
- Tax benefits under 80-IAC require separate Inter-Ministerial Board (IMB) approval  
- Startup status expires when turnover exceeds INR 200 Cr (or INR 300 Cr for DeepTech) or after 10 years (or 20 years for DeepTech)  
- DPIIT recognition does not automatically entitle a startup to all benefits; each incentive requires separate application and qualification  

#### Fee Tracking
| Item | Amount | Invoice # | Status |
|---|---|---|---|
| Retainer | [TO BE FILLED BY CONSULTANT] | INV-001 | Paid |
| Success fee | [TO BE FILLED BY CONSULTANT] | — | Pending approval |

---

## Monthly Metrics Dashboard

| Metric | This Month | Last Month | Target |
|---|---|---|---|
| New cases opened | | | |
| Applications submitted | | | |
| Approvals received | | | |
| Total value approved (₹) | | | |
| Avg. processing time (days) | [from evidence if available] | | |
| Success rate % | | | |
| Revenue — retainers | | | |
| Revenue — success fees | | | |