</think># Client Onboarding & CRM

## New Client Intake Form

**Date:** [DATE]
**Assigned consultant:** [TO BE FILLED BY CONSULTANT]
**Lead source:** [Referral / Website / Cold call / Partner]

### Business Details
(Standard fields: Legal name, Trading name, Registration type, CIN/LLPIN, Date of incorporation,
Registered address, Operational address, Website)

### Promoter / Director Details
| Name | Role | % Shareholding | PAN | Mobile | Email |
|---|---|---|---|---|---|
| | | | | | |

### Business Profile
(Standard fields: Industry/sector, NIC code, Primary products/services, Current annual turnover, No. of employees, Existing registrations)

Note which registrations are specifically required for this scheme based on evidence.
- Registration type: Company, partnership, proprietorship, or cooperative society (as per eligibility)
- Required registrations: Certificate of incorporation or registration of the applicant; PAN card; land title documents or lease agreement; environmental clearance (if applicable); undertaking regarding compliance with SEZ Act and Rules; banker's certificate regarding financial capability

### Needs Assessment
- What outcome are you seeking? (tick all that apply — list options relevant to THIS scheme's benefit type)
  - Duty-free import/domestic procurement of goods for development, operation, and maintenance
  - Exemption from central and state taxes and levies as approved by the Board of Approval
  - Exemption from minimum alternate tax under Section 115JB of the Income Tax Act
  - External commercial borrowing up to USD 500 million per year without maturity restriction
  - Single window clearance for approvals
  - Exemption from industrial licensing
  - Exemption from service tax
- Investment planned in next 12 months:
- Any pending government applications? Details:
- Any past rejected applications? What scheme and why:

### Compliance Status
(Table of documents — include ALL documents required by this scheme from evidence.
Mark which are critical vs. optional.)

| Document | Available? | Year/Period | Notes |
|---|---|---|---|
| Project report detailing the SEZ proposal | | | Critical |
| Certificate of incorporation or registration of the applicant | | | Critical |
| PAN card of the applicant | | | Critical |
| Board resolution authorizing the application | | | Critical |
| Details of promoters and directors | | | Critical |
| Land title documents or lease agreement | | | Critical |
| Environmental clearance (if applicable) | | | Optional (only if applicable) |
| Undertaking regarding compliance with SEZ Act and Rules | | | Critical |
| Banker's certificate regarding financial capability | | | Critical |

### Engagement Terms
- Retainer fee: [TO BE FILLED BY CONSULTANT]
- Success fee: [TO BE FILLED BY CONSULTANT]
- Target schemes: Special Economic Zone (SEZ) Scheme
- Engagement start date:
- Expected application date: (Rolling basis — applications can be submitted throughout the year.)

### Communication Preferences
(Standard — primary contact, preferred channel, update frequency)

---

## CRM Stage Pipeline

(Standard 10-stage pipeline — Lead through Disbursed/Rejected.
Add a scheme-specific note at the "In preparation" stage about this scheme's DPR/portal requirements.)

| Stage | Description | Action |
|---|---|---|
| 01 - Lead | Initial contact made | Schedule discovery call |
| 02 - Qualified | Eligibility confirmed for Special Economic Zone (SEZ) Scheme | Send proposal |
| 03 - Engaged | Retainer signed | Start onboarding |
| 04 - Docs collection | Gathering documents | Chase missing items |
| 05 - In preparation | Drafting application on https://www.sezindia.nic.in | Internal review |
| 06 - Submitted | Application filed on https://www.sezindia.nic.in | Begin follow-up |
| 07 - Under review | With Department of Commerce, Ministry of Commerce and Industry | Monitor + respond to queries |
| 08 - Approved | Sanction letter received | Invoice success fee |
| 08 - Approved | Sanction letter received | Invoice success fee |
| 09 - Disbursed | Money received by client | Close case, request testimonial |
| 10 - Rejected | Application declined | Analyze, re-apply or alternate scheme |