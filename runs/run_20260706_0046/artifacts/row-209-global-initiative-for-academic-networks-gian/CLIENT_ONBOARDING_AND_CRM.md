# Client Onboarding & CRM

## New Client Intake Form

**Date:** [DATE]
**Assigned consultant:** [TO BE FILLED BY CONSULTANT]
**Lead source:** [Referral / Website / Cold call / Partner]

### Business Details
(Standard fields: Legal name, Trading name, Registration type, CIN/LLPIN, Date of incorporation,
Registered address, Operational address, Website)

### Promoter / Director Details
| Name | Role | % Shareholding | PAN | Mobile | Email |
|---|---|---|---|---|---|
| | | | | | |

### Business Profile
(Standard fields: Industry/sector, NIC code, Primary products/services, Current annual turnover, No. of employees, Existing registrations)

Note which registrations are specifically required for this scheme based on evidence.
- Government (State or Central) higher education Institution / University
- Must be in top 200 in NIRF overall rankings
- Must have at least NAAC ‘A’ grade (3.0 and above)

### Needs Assessment
- What outcome are you seeking? (tick all that apply — list options relevant to THIS scheme's benefit type)
  - International faculty engagement
  - Knowledge exchange
  - Collaborative research opportunities
  - Course development
 
  Pedagogic innovation
  - Global exposure for students and faculty
  - Access to international expertise
  - Creation of digital repository for future viewing via National Digital Library
  - Potential for MOOC creation
  - Honorarium and travel support for foreign faculty
  - Honorarium for host faculty and local coordinator
  - Video recording and webcasting facilities
  - Online feedback mechanism
- Investment planned in next 12 months:
- Any pending government applications? Details:
- Any past rejected applications? What scheme and why:

### Compliance Status
(Table of documents — include ALL documents required by this scheme from evidence.
Mark which are critical vs. optional.)

| Document | Available? | Year/Period | Notes |
|---|---|---|---|
| Formal request letter from the highest Authority (VC / Director) of the Institute | | | Critical |
| NAAC ‘A’ grade certificate | | | Critical |
| Proof of NIRF Ranking in top 200 | | | Critical |
| Formal office order for appointment of local coordinator, GIAN of the Institute/University (including name, email address and mobile number) | | | Critical |

### Engagement Terms
- Retainer fee: [TO BE FILLED BY CONSULTANT]
- Success fee: [TO BE FILLED BY CONSULTANT]
- Target schemes: Global Initiative for Academic Networks (GIAN)
- Engagement start date:
- Expected application date: (note deadlines from evidence) Proposals may be submitted throughout the year. Three months required for review to final approval. Preferably, six months lead time from date of holding the course.

### Communication Preferences
(Standard — primary contact, preferred channel, update frequency)

---

## CRM Stage Pipeline

(Standard 10-stage pipeline — Lead through Disbursed/Rejected.
Add a scheme-specific note at the "In preparation" stage about this scheme's DPR/portal requirements.)

| Stage | Description | Action |
|---|---|---|
| 01 - Lead | Initial contact made | Schedule discovery call |
| 02 - Qualified | Eligibility confirmed for Global Initiative for Academic Networks (GIAN) | Send proposal |
| 03 - Engaged | Retainer signed | Start onboarding |
| 04 - Docs collection | Gathering documents | Chase missing items |
| 05 - In preparation | Drafting application on https://www.gian.iitkgp.ac.in/GREGN/ | Internal review |
| 06 - Submitted | Application filed on https://www.gian.iitkgp.ac.in/GREGN/ | Begin follow-up |
| 07 - Under review | With Ministry of Education (formerly Ministry of Human Resource Development) | Monitor + respond to queries |
| 08 - Approved | Sanction letter received | Invoice success fee |
| 09 - Disbursed | Money received by client | Close case, request testimonial |
| 10 - Rejected | Application declined | Analyze, re-apply or alternate scheme |