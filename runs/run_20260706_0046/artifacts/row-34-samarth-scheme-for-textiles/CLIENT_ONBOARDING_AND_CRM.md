# Client Onboarding & CRM

## New Client Intake Form

**Date:** [DATE]
**Assigned consultant:** [TO BE FILLED BY CONSULTANT]
**Lead source:** [Referral / Website / Cold call / Partner]

### Business Details
(Standard fields: Legal name, Trading name, Registration type, CIN/LLPIN, Date of incorporation,
Registered address, Operational address, Website)

### Promoter / Director Details
| Name | Role | % Shareholding | PAN | Mobile | Email |
|---|---|---|---|---|---|
| | | | | | |

### Business Profile
(Standard fields: Industry/sector, NIC code, Primary products/services, Current annual turnover, No. of employees, Existing registrations)

Note which registrations are specifically required for this scheme based on evidence.
- Registration for Empanelment under 'Samarth' (SCBTS) is mandatory for Implementing Agencies
- Registration for Textiles Industry/Associations registered under Central or State Government/Chambers of Commerce is required for industry applicants

### Needs Assessment
- What outcome are you seeking? (tick all that apply — list options relevant to THIS scheme's benefit type)
  - Demand-driven, placement oriented NSQF compliant skilling programmes
  - Placement linked skilling with mandatory wage employment (70% in organized sector, 50% in traditional sector)
  - Post-placement tracking for one year
  - Third party assessment and certification
  - Aadhaar enabled biometric attendance system with minimum 80% attendance
  - CCTV recording for entire training programme and assessment process
  - Training by certified trainers having ToT certification by RSA/SSCs
  - Robust MIS and real-time scheme information publicized
  - Call centre (Helpline) for feedback
  - Mobile app for stakeholders
  - Preference for marginalized social groups and 115 aspirational districts
  - Public grievance redressal with designated officer
  - Internal complaints committee under Sexual Harassment Act
  - Funding as per MSDE norms
  - Common branding as per MSDE guidelines
  - Concessional credit under Pradhan Mantri MUDRA Yojana for self-employment
- Investment planned in next 12 months:
- Any pending government applications? Details:
- Any past rejected applications? What scheme and why:

### Compliance Status
(Table of documents — include ALL documents required by this scheme from evidence.
Mark which are critical vs. optional.)

| Document | Available? | Year/Period | Notes |
|---|---|---|---|
| Letter of Proposal Submission | | | Critical |
| Authorized Signatory Letter | | | Critical |
| Request Letter | | | Critical |
| Identity Proof document | | | Critical |
| Identity number | | | Critical |

### Engagement Terms
- Retainer fee: [TO BE FILLED BY CONSULTANT]
- Success fee: [TO BE FILLED BY CONSULTANT]
- Target schemes: SAMARTH Scheme for Textiles
- Engagement start date:
- Expected application date: (note deadlines from evidence)
  - Please upload action plan (project wise) in MIS till 29-01-2024 under tab 'Notice'

### Communication Preferences
(Standard — primary contact, preferred channel, update frequency)

---

## CRM Stage Pipeline

(Standard 10-stage pipeline — Lead through Disbursed/Rejected.
Add a scheme-specific note at the "In preparation" stage about this scheme's DPR/portal requirements.)

| Stage | Description | Action |
|---|---|---|
| 01 - Lead | Initial contact made | Schedule discovery call |
| 02 - Qualified | Eligibility confirmed for SAMARTH Scheme for Textiles | Send proposal |
| 03 - Engaged | Retainer signed | Start onboarding |
| 04 - Docs collection | Gathering documents | Chase missing items |
| 05 - In preparation | Drafting application on https://samarth-textiles.gov.in/ | Internal review |
| 06 - Submitted | Application filed on https://samarth-textiles.gov.in/ | Begin follow-up |
| 07 - Under review | With Ministry of Textiles, Government of India | Monitor + respond to queries |
| 08 - Approved | Sanction letter received | Invoice success fee |
| 09 - Disbursed | Money received by client | Close case, request testimonial |
| 10 - Rejected | Application declined | Analyze, re-apply or alternate scheme |